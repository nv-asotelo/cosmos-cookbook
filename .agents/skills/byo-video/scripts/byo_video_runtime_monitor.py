...
ERROR_RULES = [
    ...
    },
    {
        ...
        "fix": "Stale gradio_cr2_byo.py on the remote. Redeploy from the bundled byo-video scripts directory.",
    },
    {
...
def poll_once(remote, rate, session_id, model_id):
    ...
    bundle = (
        ...
        f"echo '---LOGTAIL---'; tail -c {LOG_BYTE_CAP} /tmp/gradio_demo.log 2>/dev/null || true; "
        f"echo '---RESULT_MTIME---'; stat -c %Y {RESULT_JSON_PATH} 2>/dev/null || echo 0; "
        f"echo '---RESULT---'; cat {RESULT_JSON_PATH} 2>/dev/null || true; "
        "exit 0"
    )

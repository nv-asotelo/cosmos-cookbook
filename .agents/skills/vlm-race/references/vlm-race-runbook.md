...
| `brev login` fails with EOF | `brev login` requires a browser handoff — it cannot run in a non-TTY agent shell. Open a separate terminal tab, run `brev login` there, complete the browser prompt, then return to the agent. |
...
```bash
Tell the user directly when VLM Race is done and the instance can be terminated.
```

...
4. Run setup (streams live output here):
   ...
   ```
5. URLs print at the end. Reason Vite writes `/tmp/nvidia_build_reason_vite_url.txt`; Predict Vite writes `/tmp/nvidia_build_predict_vite_url.txt`; optional Next.js captures can write `/tmp/nvidia_build_reason_next_url.txt` and `/tmp/nvidia_build_predict_next_url.txt`; Gradio is always written to `/tmp/gradio_url.txt`; batch-inference/FiftyOne companion UI is written to `/tmp/byo_video_batch_inference_url.txt` when selected or when `BYO_VIDEO_LAUNCH_BATCH_INFERENCE=1` is set. Read all back with `cat /tmp/nvidia_build_reason_vite_url.txt 2>/dev/null; cat /tmp/nvidia_build_predict_vite_url.txt 2>/dev/null; cat /tmp/nvidia_build_reason_next_url.txt 2>/dev/null; cat /tmp/nvidia_build_predict_next_url.txt 2>/dev/null; cat /tmp/gradio_url.txt; cat /tmp/byo_video_batch_inference_url.txt 2>/dev/null` via ssh.

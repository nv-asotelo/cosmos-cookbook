#!/usr/bin/env python3
"""Friendly CLI companion for the BYO-video Batch Inference UI.

The HTML Batch Inference UI remains the expert surface. This guide is
...
def request_json(url: str, path: str, payload: Optional[Dict[str, Any]] = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    try:
        ...
        raise GuideError(f"Could not reach Batch Inference UI at {full_url}: {exc}")
...
def wizard(url: str) -> None:
    ...
    while True:
        ...
        try:
            ...
            print(f"\nProblem: {exc}")
            print("The Batch Inference UI kept the run from proceeding or surfaced the backend error. Adjust settings, load fewer videos, or ask Claude to inspect the logs.")

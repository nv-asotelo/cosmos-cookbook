...
> **Authors:** Kyungchan Son, Minsoo Song, Yujeong Jeong, and [Yuri Rocha](https://huggingface.co/yurirocha15) -- Doosan Robotics
...
3. Put the token in `docker/.env` as `HF_TOKEN=...`, or export it before launching so `docker/launch.sh` can copy it into `.env`.
...
| `VLLM_MAX_MODEL_LEN` | Context length. Keep this >= 5120 because the app sends ~2200 input tokens and asks for up to 2048 output tokens | `5120` |
...
|--------|-----------------|---------|
| `PICK_AND_PLACE` | `box`, `target_pallet`, `position`, `speed_pct`, `grip_strength`, `reason` | Pick one visible box and place it at a valid pallet grid cell |
...
```bash
make init       # uv sync, CUDA torch install, adapters
...
| vLLM returns 400 for prompt length | `VLLM_MAX_MODEL_LEN` too low | Keep `VLLM_MAX_MODEL_LEN >= 5120` |
...
- `make docker-test` validates plumbing, not Cosmos Reason 2 behavior.
...
- Add per-step artifact logging with `STEP_LOG_DIR` in the app environment if you want reproducible debugging traces.
